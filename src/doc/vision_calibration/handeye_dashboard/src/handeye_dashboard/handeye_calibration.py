...
class HandEyeCalibration(Plugin):
    ...
    def __init__(self, context):
        ...
        self.camera_frame.setText("camera_color_frame")
        ...
        self.base_frame.setText("base_link")
        ...
        self.endeffector_frame.setText("link_6")
    ...
    def calibration(self):
        ...
        try:
            ...
            file_output = './' + 'camera-robot.json'
            with open(file_output, 'w') as f:
                json.dump(bTc.tolist(), f)

            self.textedit.append(f"result:{np.array2string(bTc, separator=',')}\n")
            ...
            self.textedit.append(f"quaternion(x,y,z,w):")
            for item in quaternion:
                self.textedit.append(f"- {item}")
            self.textedit.append(f"translation (x,y,z):")
            for item in translation:
                self.textedit.append(f"- {item}")

        except Exception as e:
            self.textedit.append(f"Failed to solve the hand-eye calibration for {e}.")

    def execution(self):
        # >>> Publish the camera-robot transform
        self.textedit.append('Publishing the camera TF ...')
        file_input = './' + 'camera-robot.json'
